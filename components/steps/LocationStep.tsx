...
import { MapPin, Globe, ChevronRight, ChevronLeft, Sparkles } from 'lucide-react';
...
const LocationStep: React.FC<LocationStepProps> = ({ data, updateData, onNext, onBack }) => {
    const isReady = data.country && data.state;
                ...
                <div className="flex flex-col items-center justify-center relative z-10">
                    ...
                    <div className="text-center mb-12">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-50/50 border border-blue-100 text-blue-700 text-[10px] font-black tracking-[0.2em] uppercase mb-4 shadow-sm">
                            <Sparkles className="w-3 h-3" /> Step 4: Region Discovery
                        </div>
                        <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tighter leading-tight mb-4">
                            Map your <span className="text-blue-600 italic">Operations</span>
                        </h1>
                        <p className="text-slate-500 text-base font-medium max-w-md mx-auto">
                            Knowing your location helps Arkle identify specific government benefits and tax structures for you.
                        </p>
                    </div>
                    ...
                    <div className="w-full max-w-md space-y-5 mb-12">
                        <div className="relative">
                            <Globe className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-blue-600/40" />
                            <select 
                                className="w-full pl-12 pr-4 py-4 bg-slate-50/50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-blue-500/20 focus:outline-none font-bold text-slate-700 appearance-none cursor-pointer"
                                value={data.country || ''}
                        ...
                        <div className="relative">
                            <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-blue-600/40" />
                            <input 
                                type="text" 
                    ...
                    <div className="flex items-center gap-4 w-full max-w-sm">
                        ...
                        >
                            Confirm Region <ChevronRight className="w-4 h-4" />
                        </button>
                    </div>
                </div>
            </motion.div>
