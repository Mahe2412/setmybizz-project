...
interface GlobalAccessStepProps {
    onNext: () => void;
    onBack: () => void;
}
...
export default function GlobalAccessStep({ onNext, onBack }: GlobalAccessStepProps) {
    ...
    return (
        <div className="w-full h-full flex flex-col bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 overflow-hidden relative">

            {/* Page 1: Country Selection */}
            {currentStep === 'country' && (
                <div className="flex-1 overflow-y-auto custom-scroll px-4 py-12">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                {COUNTRIES.map((country, idx) => (
                                    <button
                                        ...
                                        className="group relative bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-5 hover:bg-white/20 transition-all hover:scale-105 hover:shadow-2xl hover:shadow-blue-500/30 animate-in zoom-in duration-500"
                                        style={{ animationDelay: `${idx * 100}ms` }}
                                    >
                                        <div className="text-4xl mb-3 group-hover:scale-110 transition-transform">
                                            {country.flag}
                                        </div>
                                        <h3 className="text-white font-bold text-base mb-1">{country.name}</h3>
                                        <p className="text-blue-200 text-xs font-medium">{country.language}</p>
                                        <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <span className="material-icons text-blue-400 text-lg">arrow_forward</span>
                                        </div>
                                    </button>
                                ))}
                            </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scroll px-4 py-12">
                    <div className="min-h-full flex flex-col items-center justify-center relative">
                        ...
                        <button
                            ...
                            className="absolute top-0 left-0 md:top-6 md:left-6 flex items-center gap-2 text-white/80 hover:text-white transition-colors z-20"
                        >
                            <span className="material-icons">arrow_back</span>
                            <span className="font-bold text-sm md:text-base">Change Service</span>
                        </button>

                        <div className="max-w-4xl w-full mt-6 md:mt-0">
                            <div className="text-center mb-12">
                        ...
                        <div className="max-w-4xl mx-auto">
                            ...
                            {chatMessages.length >= 3 && (
                                ...
                                >
                                    ...
                                    Ready to Get Started? Create Your Account & Login
                                    <span className="material-icons">arrow_forward</span>
                                </button>
                            )}
                        </div>
